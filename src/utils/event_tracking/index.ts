            ...
            [25, 50, 75, 100].forEach((milestone) => {
                if (scrollPercent >= milestone && !scrollMilestones[milestone as keyof typeof scrollMilestones]) {
                    if (category === undefined && slug === undefined) {
                        ...
                        const match = LEARN_MORE_ITEMS.find(item => pathname === `/learn_more/${item.id}` || (item.slug && pathname === `/learn_more/${item.slug}`));
                        if (match) {
                        ...
                        const match = RELATABLE_STORIES_ITEMS.find(item => pathname === `/relatable_stories/${item.id}` || (item.slug && pathname === `/relatable_stories/${item.slug}`));
                        if (match) {
                        ...
                        const match = CLIENT_SESSIONS_ITEMS.find(item => pathname === `/client_sessions/${item.id}` || (item.slug && pathname === `/client_sessions/${item.slug}`));
                        if (match) {
